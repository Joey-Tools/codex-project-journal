...
- Repository discovery streams active `sessions` and flat `archived_sessions` through one shared absolute deadline and aggregate filesystem-entry, unique-rollout, total-byte, line-byte, record, normalized-distinct-CWD, JSON-depth, and retained-error caps. Duplicate active/archive rollout basenames count once. Require `coverage_status: complete` before treating the candidate set as complete; `partial` preserves already found rows but adds structured `discovery_coverage`, and a partial scan with no repository emits an inconclusive sentinel instead of `[]`.
