...
- Before Git runtime initialization and command dispatch, the helper temporarily defers `SIGHUP`, `SIGTERM`, and `SIGQUIT`. The handler records only the first signal and performs no filesystem or process cleanup. Bounded normal-control-flow checkpoints raise that request through the existing `BaseException` cleanup paths, so launch-owned process groups are drained, hook rename state is revalidated and reported, and the process-private Git runtime snapshot is released or reported with a retained locator first. A final post-restoration check routes a request captured while handlers were being restored through the same runtime-cleanup commit before reporting and propagating the original signal semantics; ignored handlers stay ignored, and a restored custom handler may return.
