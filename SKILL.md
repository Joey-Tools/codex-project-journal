...
- In `discover-repos` output, treat `adoption_status` as the authoritative tag and require it to be `adopted` or `unadopted` before consuming adoption evidence. `inconclusive` carries a structured `adoption_error` and null adoption fields; it is not evidence that the repository is unadopted. Auxiliary worktree failures are isolated to that row through `discovery_status` and `discovery_error`.
...
- Before installing into the default hook directory, the helper inspects system and global Git configuration without following includes and refuses installation when either scope sets `core.hooksPath` or contains an unresolved include. Set an explicit repo-local `core.hooksPath` and rerun when instructed. Installation binds one validated hook-directory descriptor, opens existing targets with `O_NOFOLLOW`, stages owner-private files in that same directory, atomically replaces targets relative to the descriptor, and revalidates directory identity and access policy.
