...
- Before any repository Git read, the helper snapshots one resolved Git executable into an owner-private directory and runs the bounded credential-free `git --version` gate against those exact bytes; Git older than 2.45 fails closed. It revalidates the snapshot identity and SHA-256 before every repository command, clears ambient Git control variables, disables external Git configuration and lazy object fetching, and never returns to the mutable source executable.
- Adoption validation binds the initial raw index snapshot, one bounded `git cat-file --batch` session, frontmatter parsing, semantic validation, valid-entry classification, and final raw index revalidation to one absolute monotonic deadline. It validates exact OID/type/size/content framing, invalidates every member of a duplicate-ID group, and enforces index, entry, frontmatter field/list, validation-issue, byte, record, and stderr limits. Per-path validation state is structured; it does not recover validity by rescanning formatted issue strings.
...
- In `discover-repos` output, treat `adoption_status` as the authoritative tag and require it to be `adopted` or `unadopted` before consuming adoption evidence. `inconclusive` carries a structured `adoption_error` and null adoption fields; it is not evidence that the repository is unadopted. Auxiliary worktree failures are isolated to that row through `discovery_status` and `discovery_error`; they null only affected auxiliary fields and do not overwrite authoritative index adoption.
...
- Hook installation is POSIX-only. Before installing into the default hook directory, the helper inspects system and global Git configuration without following includes and refuses installation when either scope sets `core.hooksPath` or contains an unresolved include. Set an explicit repo-local `core.hooksPath` and rerun when instructed. Installation binds the complete absolute path from the filesystem root, traverses and creates allowed relative components with descriptor-relative `O_NOFOLLOW` operations, retains and revalidates every ancestor identity/access policy, and opens existing targets with `O_NOFOLLOW`. It stages owner-private files in the same directory and commits with native no-replace or exchange rename semantics; an absent, replaced, or content-changed target fails closed, and a mismatched exchanged entry is rolled back. Target revalidation protects path and target object identity, type, ownership, access mode, size, and content while allowing timestamp-only transitions. This contract detects ordinary concurrent installer races; it does not claim to exclude an actively malicious same-UID process that ignores the protocol and mutates entries after a final verified kernel operation.
