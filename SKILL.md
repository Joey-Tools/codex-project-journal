...
- Before any repository Git read, the helper opens one resolved Git executable with required `O_NOFOLLOW|O_NONBLOCK`, binds the selected pathname and descriptor to the same initial regular-file identity and access policy, and copies stable content under a 64 MiB ceiling and one monotonic time budget into an owner-private snapshot. It runs the bounded credential-free `git --version` gate against those exact bytes; Git older than 2.45 fails closed. It revalidates the snapshot identity and SHA-256 before every repository command, clears ambient Git control variables, disables external Git configuration and lazy object fetching, and never returns to the mutable source executable. The copy-stage deadline prevents blocking on a substituted FIFO but does not claim to preempt one stalled regular-file kernel operation.
