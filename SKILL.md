...
- Before installing into the default hook directory, the helper inspects system and global Git configuration without following includes and refuses installation when either scope sets `core.hooksPath` or contains an unresolved include. Set an explicit repo-local `core.hooksPath` and rerun when instructed. Installation starts from a verified allowed repository or Git-root descriptor, traverses and creates every relative component with descriptor-relative `O_NOFOLLOW` operations, retains and revalidates every ancestor identity/access policy, opens existing targets with `O_NOFOLLOW`, stages owner-private files in that same directory, and atomically replaces targets relative to the descriptor. Target revalidation protects object identity, type, ownership, access mode, size, and content while allowing timestamp-only transitions.
