...
- The helper clears ambient Git control variables, disables external Git configuration and lazy object fetching, and binds the initial raw index snapshot, one bounded `git cat-file --batch` session, and final raw index revalidation to one absolute monotonic deadline. It validates the exact requested OID/type/size/content framing and enforces index byte/record/stderr plus per-blob and aggregate byte limits. On POSIX, the direct child stays unreaped as the PID/PGID identity fence while status is observed with `WNOWAIT`; every bound process-group probe and signal finishes before the final reap, and no numeric PGID is signalled after that fence is released. A missing fence or unverified final cleanup is reported as `cleanup-incomplete`. A redirected, incomplete, malformed, changed, over-limit, or cleanup-incomplete validation fails closed.
